...
corpus = 'data\corpus_enem.xml'

def registra_redacao(row, tipo):
    obj_txt = Texto(row['texto'], row['avaliacao'], tipo)
    obj_txt.to_xml(corpus)
...
df_treino.apply(lambda x: registra_redacao(x, tipo = 'treino'), axis=1)
df_teste.apply(lambda x: registra_redacao(x, tipo = 'teste'), axis=1)